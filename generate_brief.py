"""
Daily Intelligence Brief Generator — v8.2
...
v8.2 upgrades:
  1. Macro Physics Engine: real yield computed in Python, injected as hard_truths
  2. Logic Guardrail: Haiku cross-checks draft against hard_truths before Narrator
  3. Show, Don't Tell: Narrator forbidden from meta-language
  4. Periphery Decoupling: explicitly allowed to state no causal link
  5. Conviction locked: H/M/L format strictly defined
  6. Narrator reframed as 通識課教授 (liberal arts professor)
  7. Correlation matrix in Analyst context
...
def format_market_data(yfd: dict, frd: dict, corr_text: str) -> tuple[str, dict]:
    ...
    hard_truths contains Python-computed facts for the Logic Guardrail."""
    ...
    if frd:
        ...
        # Real Yield = Nominal 10Y - Breakeven (Python-computed, not LLM-inferred)
...
def perform_logic_guardrail(analyst_draft: str, hard_truths: dict) -> str:
    """Haiku cross-checks Analyst draft against Python-computed hard_truths.
    Returns empty string if PASS, or a correction warning string."""
    if not hard_truths:
        return ""

    check_prompt = f"""你是嚴格的數據審核員。

以下是今日 Python 直接計算的硬核數據事實：
{json.dumps(hard_truths, ensure_ascii=False, indent=2)}
...
請只檢查以下兩件事：
1. 草稿中對「實質利率（Real Yield）」變動方向的描述，是否與 real_yield_direction 一致？
2. 草稿中對「殖利率曲線」形態的描述，是否與 yield_curve_inverted 一致？

若兩項均正確，回覆：PASS
若有衝突，用一句話指出錯誤：[哪個指標] 草稿說 [X]，但數據顯示 [Y]。
只輸出結論，不要解釋。"""
...
def perform_logic_review(analyst_draft: str) -> str:
    ...
    review_prompt = f"""以下是今日情報草稿，請審查邏輯鏈縝密性：
...
# ── Narrator（通識課教授）────────────────────────────────────────────────────
NARRATOR_SYSTEM = """你是一位宏觀對沖基金背景出身的通識課教授。

你的三重身份：
- 分析師：把複雜的市場機制翻譯成精準的判斷
- 教授：引導學生自己想清楚，不直接給答案，讓問題比答案更有價值
- 作者：每一句話都有它存在的理由，沒有廢話

讀者：政治學、經濟學背景的學生，志在金融與建立全球視野，已有分析肌肉，缺乏實戰練習量。
...
def build_narrator_prompt(analyst_draft: str, guardrail_warning: str = "", logic_review: str = "") -> str:
    correction = ""
...
def main():
    ...
    # ── 2.5 Logic Guardrail (Haiku) ───────────────────────────────────────────
    print("  → [Guardrail] Macro logic check (Haiku)...")
    guardrail_warning = perform_logic_guardrail(analyst_draft, hard_truths)
    if guardrail_warning:
        print("    ⚠ Logic violation detected — correction injected into Narrator")
    else:
        print("    ✓ Logic check passed")

    # ── 2.6 Logic Chain Review (Opus) ─────────────────────────────────────────
    print("  → [Logic Reviewer] Deep logic chain review (Opus)...")
    logic_review = perform_logic_review(analyst_draft)
    if logic_review:
        print(f"    ⚠ Logic issues found — injected into Narrator")
        print(f"    Preview: {logic_review[:120]}...")
    else:
        print("    ✓ Logic chain review passed")
    ...
    final_report = with_retry(
        call_claude,
        NARRATOR_SYSTEM,
        build_narrator_prompt(analyst_draft, guardrail_warning, logic_review),
        MODEL_SONNET,
        max_tokens=5500,
    )
