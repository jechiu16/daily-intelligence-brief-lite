...
NOTION_DB_ID      = os.environ["NOTION_DATABASE_ID"]   # Daily reports database
NOTION_WEEKLY_DB  = os.environ.get("NOTION_WEEKLY_DB_ID", NOTION_DB_ID)  # Weekly DB (optional, falls back to same DB)
...
def markdown_to_notion_blocks(md: str) -> list:
    ...
    while i < len(lines):
        ...
        if line.startswith("# "):
            ...
            # Regular paragraph — chunk to ≤2000 chars (Notion limit)
            text = line.strip()
            if text:
                for chunk_start in range(0, len(text), 2000):
                    blocks.append({"object":"block","type":"paragraph","paragraph":{"rich_text":[{"type":"text","text":{"content":text[chunk_start:chunk_start+2000]}}]}})
        i += 1
...
def main():
    ...
    # 2. If today is Monday, also generate weekly review
    if IS_MONDAY:
